...
def call_llm(prompt: str, use_cache: bool = True) -> str:
    ...
    if not provider:
        raise ValueError(
            "No LLM provider configured. Set one of: (1) GEMINI_API_KEY or GEMINI_PROJECT_ID for Google Gemini, "
            "or (2) LLM_PROVIDER plus <PROVIDER>_MODEL and <PROVIDER>_BASE_URL for OpenAI-compatible APIs (e.g. OLLAMA, XAI)."
        )
    if provider == "GEMINI":
